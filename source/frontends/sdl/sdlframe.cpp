...
namespace sa2
{
    ...
    void SDLFrame::SetApplicationIcon()
    {
        ...
        std::shared_ptr<SDL_IOStream> ops(SDL_IOFromConstMem(resource.first, resource.second), SDL_CloseIO);
        std::shared_ptr<SDL_Surface> icon(IMG_Load_IO(ops.get(), 1), SDL_FreeSurface);
        if (icon)
        {
